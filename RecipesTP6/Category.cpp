	...
	: AbsCatalogComponent(mdd.m_name)
{
	...
	for (auto&& elem : mdd.m_products)
	{
		addCatalogComponent(*elem);
	}
}
...
AbsCatalogComponent& Category::addCatalogComponent(const AbsCatalogComponent & member)
{
	...
	m_products.push_back(CatalogComponentPtr(component));
...
const AbsProduct* Category::findProduct(std::string productName) const
{
	...
	for (auto&& elem : m_products)
	{
		const AbsProduct* foundProduct = elem->findProduct(productName);
		if (elem != nullptr)
		{
			return foundProduct;
		}
...
std::ostream & Category::printToStream(std::ostream & o) const
{
	...
	o << "Categorie : " << m_name << std::endl;
	for (auto&& produit : m_products)
	{
		m_indent++;
		indent(o);
		o << *produit << std::endl;
